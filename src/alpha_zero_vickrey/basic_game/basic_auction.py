...
class BasicAuctionGame(Game):
    ...
    def getActionSize(self):
        ...
        return 11 # prob vector dimensions
    ...
    def sanitizeMoves(self, state, pi):
        game = AuctionEnv(state=state)
        return game.sanitize_moves(pi)


    def getMaxBid(self, state):
        game = AuctionEnv(state=state)
        return game.get_max_bid()
    
    def getMustBid(self, state):
        game = AuctionEnv(state=state)
        return game.get_must_bid()
