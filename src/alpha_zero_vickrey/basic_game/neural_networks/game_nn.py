...
class AuctionEnvNet(nn.Module):
    def __init__(self, game, pool_seq_length_forward=None, pool_seq_length_defense=None, pool_seq_length_goalie=None):
        ...
        # --------------------------
        # Global Branch:
        # – Features: [forwards_left, defense_left, goalies_left,
        #            nominated_player_mean, nominated_player_position(one-hot 3-d)]
        # Total input dim = 3 + 1 + 3 = 7.
        global_input_dim = 7
        global_hidden_dim = 12
        self.global_branch = nn.Sequential(
            nn.Linear(global_input_dim, global_hidden_dim),
            nn.ReLU(),
            nn.Linear(global_hidden_dim, 12),
            nn.ReLU()
        )
        ...
        opp_input_dim = 5
        opp_hidden_dim = 12  # shared for each opponent
        ...
        merged_dim = 12 + 5 + 5 + 24
        self.merged_fc = nn.Sequential(
            nn.Linear(merged_dim, 12),
            nn.ReLU(),
            nn.Linear(12, 12),
        )
        ...
        self.policy_head = nn.Sequential(
            nn.Linear(12, 12),
            nn.ReLU(),
            nn.Linear(12, game.getActionSize())  # rough image of a bid distribution - we will make use of this elsewhere
        )
        
        self.value_head = nn.Sequential(
            nn.Linear(12, 5),
            nn.ReLU(),
            nn.Linear(5, self.num_players),  # scalar value
            nn.Softmax(dim=1)
        )
    ...
    def forward(self, state):
        ...
        For example, state might contain:
          - global: a tensor of shape (batch, 7) for the global branch.
        ...
        """
        # Global branch
        global_input = state["global"]  # shape (B, 7)
        ...
        pi = self.policy_head(merged)  # (B, 11)
