...
class VanillaMCTS():
    ...
    def random_playout(self, state):
        ...
        while True:
            ...
            action = random.randint(0, self.game.getActionSize() - 1) #self.game.sanitizeMoves(current_state, np.ones(self.game.getActionSize()))
    ...
    def vanilla_monte_carlo(self, state):
        ...
        pi = np.ones(self.game.getActionSize()) #$self.game.sanitizeMoves(state, np.ones(self.game.getActionSize()))
        pi /= np.sum(pi)

        actions = [i for i,prob in enumerate(pi) if prob != 0]
        ...
        print(current_player, average_rewards, state["nominated_player"])
...
class MCTS():
    ...
    def getActionProb(self, state, temp=1):
        ...
        #q_vals = {a : self.Qsa[(s,a)] for a in range(self.game.getActionSize())}
