...
from utils import sample_prob_vec
...
class Coach():
    ...
    def executeVanillaEpisode(self):
        ...
        while True:
            ...
            action = sample_prob_vec(pi)
            #print(f"Current player {self.curPlayer} is bidding {action}")
        ...
        while True:
            ...
            if type(r) is not int:
                ...
                for x in trainExamples:
                    output.append((x[0], x[2], r))
                    #print(x[2], new_r)


                #print("SAMPLE", choice(output))
                return output
    ...
    def learn(self):
        ...
        players.append(lambda x : random.randint(0,10))
        print("0 is MCTS v0 and 1 is random")
        arena = Arena(players, self.game)
        results = arena.playGames(1)
        print(results)
        ...
        for i in range(2, self.args.numIters + 2):
            ...
            for i in range(10):
                ...
                sample = random.choice(trainExamples)
                print(sample)
                print()
                print()
                x = self.game.prepare_tensor_input([sample[0]], device="mps")
                print(x)
                print()
                print()
                print(MCTS(self.game, self.nnet, self.args).getActionProb(sample[0], temp=0))

            if i > -1:
                if i == 11:
                    self.pnet.load_checkpoint(folder=self.args.checkpoint, filename=self.getCheckpointFile(2))
