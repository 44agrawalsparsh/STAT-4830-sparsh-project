...
def prepare_batch_state(batch_states, 
    ...
    for state in batch_states:
        ...
        gl.append(state["current_bid"] / game_budget)
        ...
        our_forwards_needed = state["members_forwards_needed"][0] / forwards_needed
        our_defense_needed  = state["member_defense_needed"][0] / defense_needed
        our_goalies_needed  = state["member_goalies_needed"][0] / goalies_needed
        our_budget          = state["budgets"][0] / game_budget
        our_cumulative_value= state["members_means"][0] / expected_team_value
        our_vec = [
            our_forwards_needed,
            our_defense_needed,
            our_goalies_needed,
        ...
        opps = []
        opponent_indices = list(range(1, num_players))
        ...
        if len(norm_pool_fwd) < target_len_fwd:
            norm_pool_fwd.extend([0.0]*(target_len_fwd - len(norm_pool_fwd)))
        else:
        ...
        if len(norm_pool_def) < target_len_def:
            norm_pool_def.extend([0.0]*(target_len_def - len(norm_pool_def)))
        else:
        ...
        if len(norm_pool_goal) < target_len_goal:
            norm_pool_goal.extend([0.0]*(target_len_goal - len(norm_pool_goal)))
        else:
