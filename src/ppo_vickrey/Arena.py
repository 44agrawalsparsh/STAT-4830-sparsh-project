...
class Arena():
    ...
    def playGame(self, players, verbose=False):
        ...
        while not self.game.getGameOver(state):
            ...
            state = self.game.getNextState(state, action, print_bid=verbose)
        ...
        if verbose:
            ...
            self.display(board)
        '''
        return self.game.getRewards(state)
    ...
    def playGames(self, num, verbose=False):
        ...
        for _ in tqdm(range(num), desc="Games Time"):
            ...
            if np.random.random() < verbose:
                result = self.playGame(players,verbose=True)
            else:
                result = self.playGame(players,verbose=False)
