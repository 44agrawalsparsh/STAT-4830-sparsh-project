...
def prepare_batch_state(batch_obs, 
    ...
    for state in batch_obs:
        ...
        # Current bid normalized by game budget
        #current_bid_norm = state["current_bid"] / game_budget
        #gl.append(current_bid_norm)
        ...
        if len(norm_pool_fwd) < target_len_fwd:
            norm_pool_fwd.extend([(0 - (f_shape * f_scale)) / (f_scale * math.sqrt(f_shape))]*(target_len_fwd - len(norm_pool_fwd)))
        else:
        ...
        if len(norm_pool_def) < target_len_def:
            norm_pool_def.extend([(0 - (d_shape * d_scale)) / (d_scale * math.sqrt(d_shape))]*(target_len_def - len(norm_pool_def)))
        else:
        ...
        if len(norm_pool_goal) < target_len_goal:
            norm_pool_goal.extend([(0 - (g_shape * g_scale)) / (g_scale * math.sqrt(g_shape))]*(target_len_goal - len(norm_pool_goal)))
        else:
