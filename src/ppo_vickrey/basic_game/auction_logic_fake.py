...
INTERMEDIATE_COEF = 0.0
import random

#######################


# This is an experiment to see if training works. We're just going to change the rewards/games and see if our model learns

# #1. Remove budget constraints - you're always allowed to bid between 0 and 1000. Start with sans position constraints. In theory we should see the model quickly learnt o spam 1
# #2. Keep position constraints - not sure
# #3. Who ever is closer to bidding the players mean
    ...
    ** CHANGES **
    ...
    def __init__(self, num_players=2, budget=1000, num_forwards=10, num_defensemen=5, num_goalies=2, state=None):
        if state:
            ...
            self.last_winner = state["last_winner"]
            self.last_price = state["last_price"]
            self.prev_bids = state["prev_bids"]
    ...
    def play_action(self, action, print_bid=False):
        ''' 
        model output is desired a,b of beta distribution to sample from
        '''
        ...
        if print_bid:
            line = f"Bidder: {self.current_bidder}, Action: {action}, Bid: {bid}, Athlete: {self.nominated_player}, Budgets : {self.budgets}, Means : {self.members_means}"
            print(line)
        ...
        if next_player == 0:
            ...
            score = 1/((self.bids_placed - self.nominated_player.mean)**2)

            highest_bid_indices = np.where(score == np.max(score))[0]
            winner = np.random.choice(highest_bid_indices)
            price_paid = np.partition(self.bids_placed, -2)[-2]

            self.last_winner = winner
            self.last_price = price_paid
            self.prev_bids = copy.deepcopy(self.bids_placed)
    ...
    def get_reward(self):
        if self.game_over and 1 == 0:
            #pdb.set_trace()
            '''
            sorted_indices = np.argsort(-1*self.members_means)
            ranks = np.empty_like(sorted_indices)
            ranks[sorted_indices] = np.arange(1, self.num_players + 1)
            return 1.0/ranks
            '''

            vals = (self.members_means - np.mean(self.members_means))
            return vals

        else:
            #pdb.set_trace()
            reward = np.zeros(self.num_players)
            reward[self.last_winner] = 100

            reward = 1e3/((np.array(self.prev_bids) - self.last_price)**2 + 1)
            return reward
            ...
            for athlete in self.athletes:
                if athlete.position == AuctionEnv.Position.FORWARD:
                    worst_f = min(worst_f, athlete.mean)
                elif athlete.position == AuctionEnv.Position.DEFENSEMAN:
                    worst_d = min(worst_d, athlete.mean)
                if athlete.position == AuctionEnv.Position.GOALIE:
                    worst_g = min(worst_g, athlete.mean)
            ...
            for athlete in self.athletes:
                if athlete.position == AuctionEnv.Position.FORWARD:
                    remaining_value += athlete.mean - worst_f
                elif athlete.position == AuctionEnv.Position.DEFENSEMAN:
                    remaining_value += athlete.mean - worst_d
                if athlete.position == AuctionEnv.Position.GOALIE:
                    remaining_value += athlete.mean - worst_g

            pseudo_values = np.zeros(self.num_players)

            for i in range(self.num_players):
                pseudo_values[i] += self.members_means[i]
                
                num_f = self.FORWARDS_NEEDED - self.members_forwards_needed[i]
                pseudo_values[i] -= worst_f*num_f

                num_d = self.DEFENSEMEN_NEEDED - self.member_defense_needed[i]
                pseudo_values[i] -= worst_d*num_d

                num_g = self.GOALIES_NEEDED - self.member_goalies_needed[i]
                pseudo_values[i] -= worst_g*num_g

                pseudo_values[i] += self.budgets[i]/np.sum(self.budgets)*remaining_value

            pseudo_values /= np.mean(pseudo_values)
            pseudo_values -= np.mean(pseudo_values)
            pseudo_values *= INTERMEDIATE_COEF
            #pdb.set_trace()
            '''
            remaining_value = 0
            for athlete in self.athletes:
                remaining_value += athlete.mean

            pseudo_values = self.members_means + self.budgets/np.sum(self.budgets)*remaining_value
            pseudo_values = pseudo_values - np.mean(pseudo_values)
            return pseudo_values*INTERMEDIATE_COEF
