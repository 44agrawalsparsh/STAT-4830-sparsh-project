...
class AuctionEnv:
	...
	** CHANGES **
	...
	def generate_nomination_order(athletes):
		...
		while any(pools):
			pool = random.choice([x for x in pools if len(x)])
			nomination_order.append(pool.pop(0))

		#nomination_order.reverse()
	...
	def __init__(self, num_players=2, budget=1000, num_forwards=10, num_defensemen=5, num_goalies=2, state=None):
		if state:
			...
			self.athletes = AuctionEnv.generate_nomination_order(AuctionEnv.generate_athletes(self.num_players, self.FORWARDS_NEEDED, self.DEFENSEMEN_NEEDED, self.GOALIES_NEEDED, shape, scale))
